...
class Trainer(object):
    def __init__(self, trainloader, testloader, opt):
        ...
        self.encoder = Encoder().cuda(3)
        self.decoder = Decoder(conv_dim=64).cuda(3)
        #self.decoder_scratch = Decoder_scratch(image_size=224, conv_dim=64).cuda(3)
        #self.encoder_scratch = Encoder_scratch(image_size=224, conv_dim=64).cuda(3)

        self.discriminator = Discriminator(image_size=224, conv_dim=128).cuda(3)
        #self.discriminator_cls = Discriminator_cls(image_size=224, conv_dim=128).cuda(3)
        ...
        self.pre_cnn_path = os.path.join('/home', 'mipal', 'psw_exp', 'exp_adv','NoiseGAN', 'scratch', 'best-pre_resnet101_imgnet50.pth')
        ...
        # self.encoder.cuda(3)
        # self.decoder.cuda(3)
        # self.decoder_scratch.cuda(3)
        # self.encoder_scratch.cuda(3)
        # self.discriminator.cuda(3)
        # #self.discriminator_cls.cuda(3)
        # self.cnn.cuda(3)
        # #self.cnn.fc.cuda(3)
        # #self.imitator.cuda(3)
        # #self.imitator.fc.cuda(3)
        # self.cnn_2.cuda(3)
        # #self.cnn_2.fc.cuda(3)
        self.encoder = (self.encoder).cuda(3)
        self.decoder = (self.decoder).cuda(3)
        self.discriminator = (self.discriminator).cuda(3)
        self.cnn = (self.cnn).cuda(3)
        self.cnn_2 = (self.cnn_2).cuda(3)
    ...
    def train_classifier(self):
        ...
        for epoch in range(5):
            for i, images in enumerate(self.train_loader):
                ...
                self.cnn.zero_grad()
                images_label = Variable(images[1]).long().cuda(3)
                images = images[0].float().cuda(3)
                ...
                if (i % len(self.train_loader) == 0) and (i != 0):
                    ...
                    for im, la in self.test_loader:
                        ...
                        im_test = Variable(im, volatile=True).cuda(3)
                        label_mask = Variable(torch.zeros(self.batch_size, 10), volatile=True).cuda(3)
    ...
    def train_adversarial(self, opt):
        ...
        for epoch in range(100):
            ...
            for i, images in enumerate(self.train_loader):
                ...
                i += 1
                labels_real = Variable(torch.FloatTensor(np.ones(images[0].size(0)))).cuda(3)
                #labels_fake = Variable(torch.FloatTensor(np.zeros(self.batch_size))).cuda(3)

                image_class = Variable(images[1].cuda(3))
                np_image_class = image_class.cpu().data.numpy()
                images = images[0].cuda(3)


                labels_real = labels_real.cuda(3)
                #labels_fake = labels_fake.cuda(3)
                ...
                #clsn_mask = Variable(torch.zeros(self.batch_size)).cuda(3).long()
                #clsp_mask = Variable(torch.zeros(self.batch_size)).cuda(3).long()
                cls0_mask = Variable(torch.FloatTensor(np.zeros(images.size(0)))).cuda(3)
                cls1_mask = Variable(torch.FloatTensor(np.ones(images.size(0)))).cuda(3)
                cnt_pos = 0
                #cnt_neg = 0
                #nZeroIdx = []
                #nOneIdx = []
                ...
                #img_batch_var = Variable(torch.FloatTensor(p_img_batch.copy())).cuda(3)
                #mask_batch_var = Variable(torch.FloatTensor(p_mask_batch.copy())).cuda(3)
                ...
                if (i % len(self.train_loader) == 0) and (i!=0):
                    ...
                    for im, la in self.test_loader:
                        ...
                        label_mask = Variable(torch.zeros(la.size(0), 50), volatile=True).cuda(3)
                        label_mask_i = Variable(torch.zeros(la.size(0), 50), volatile=True).cuda(3)
                        ...
                        if j % 100 == 0:
                            torchvision.utils.save_image(img_test_resized.data.cpu(),
                                                         './data/table1_img50/res101/epoch%dimages_%d.jpg' % (epoch + 1, j))
                            torchvision.utils.save_image(mask_test.data.cpu(),
                                                         './data/table1_img50/res101/epoch%dnoise_%d.jpg' % (epoch + 1, j))
                            torchvision.utils.save_image(reconst_images.data.cpu(),
                                                         './data/table1_img50/res101/epoch%dreconst_images_%d.jpg' % (epoch + 1, j))
                    correct_meanscore /= len(self.test_loader)
                    correct_meanscore_i /= len(self.test_loader)

                    print('Test Accuracy of the cls on the masked images for resnet101: %.4f %% - %.4f %% - %.4f' % (100 * correct / total, 100* correct_o / total, correct / correct_o))
                    print('Mean Accuracy: %.4f' % correct_meanscore.data[0])
                    ...
                    if correct / total > best_score:
                        ...
                        print("saving best model...")
                        torch.save(self.encoder.state_dict(), './data/table1_img50/res101/best-encoder.pth')
                        torch.save(self.decoder.state_dict(), './data/table1_img50/res101/best-decoder.pth')
                        torch.save(self.discriminator.state_dict(), './data/table1_img50/res101/best-discriminator.pth')
                        torch.save(self.optim_G_dis.state_dict(), './data/table1_img50/res101/best-optimizer.pth')
