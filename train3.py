...
class Trainer(object):
    def __init__(self, trainloader, testloader, opt):
        ...
        self.decoder = Decoder(conv_dim=64).cuda(2)
        #self.decoder_scratch = Decoder_scratch(image_size=224, conv_dim=64).cuda(2)
        #self.encoder_scratch = Encoder_scratch(image_size=224, conv_dim=64).cuda(2)

        self.discriminator = Discriminator(image_size=224, conv_dim=128).cuda(2)
        #self.discriminator_cls = Discriminator_cls(image_size=224, conv_dim=128).cuda(2)

        #self.cnn = models.vgg16(pretrained=True)
        #self.cnn.fc = nn.Linear(self.cnn.fc.in_features, 50)
        #self.cnn.classifier._modules['6'] = nn.Linear(4096, 50)
        ...
        #self.pre_cnn_path = os.path.join('/home', 'mipal', 'NoiseGAN', 'data', 'best-pre_resnet101_stl.pth')
        self.pre_cnn_path = os.path.join('/home', 'mipal', 'psw_exp', 'exp_adv','NoiseGAN', 'scratch', 'best-pre_resnet50_imgnet50.pth')
        ...
        self.cnn_2.fc = nn.Linear(self.cnn_2.fc.in_features, 50)
        #
        #self.pre_cnn_2_path = os.path.join('/home', 'yjyoo', 'Code', 'NoiseGAN-blackbox_enhance_LS', 'data', 'best-pre_resnet50_stl.pth')
        self.pre_cnn_2_path = os.path.join('/home', 'mipal', 'psw_exp', 'exp_adv','NoiseGAN', 'scratch', 'best-pre_resnet50_imgnet50.pth')
        ...
        #self.optim_C = optim.Adam(self.cnn.classifier._modules['6'].parameters(), lr=0.0005)
        ...
        self.optim_L1 = optim.Adam(self.decoder.parameters(), lr=0.0001)
        ...
        # self.encoder.cuda(2)
        # self.decoder.cuda(2)
        # self.decoder_scratch.cuda(2)
        # self.encoder_scratch.cuda(2)
        # self.discriminator.cuda(2)
        # #self.discriminator_cls.cuda(2)
        # self.cnn.cuda(2)
        # #self.cnn.fc.cuda(2)
        # #self.imitator.cuda(2)
        # #self.imitator.fc.cuda(2)
        # self.cnn_2.cuda(2)
        # #self.cnn_2.fc.cuda(2)
        self.encoder = (self.encoder).cuda(2)
        self.decoder = (self.decoder).cuda(2)
        self.discriminator = (self.discriminator).cuda(2)
        self.cnn = (self.cnn).cuda(2)
        self.cnn_2 = (self.cnn_2).cuda(2)
    ...
    def finetune(self, allow=True):
        ...
        #for param in self.cnn.classifier._modules['6'].parameters():
    ...
    def train_classifier(self):
        ...
        for epoch in range(5):
            for i, images in enumerate(self.train_loader):
                i += 1
                # self.cnn.classifier._modules['6'].zero_grad()
                self.cnn.zero_grad()
                images_label = Variable(images[1]).long().cuda(2)
                images = images[0].float().cuda(2)
                ...
                if (i % len(self.train_loader) == 0) and (i != 0):
                    ...
                    for im, la in self.test_loader:
                        ...
                        im_test = Variable(im, volatile=True).cuda(2)
                        label_mask = Variable(torch.zeros(self.batch_size, 10), volatile=True).cuda(2)
                    ...
                    correct_meanscore /= (8000/50)  # 200 = number of iteration in one test epoch
                    ...
                    #     torch.save(self.cnn.state_dict(), './data/best-pre_densenet.pth')
    ...
    def train_adversarial(self, opt):
        ...
        for epoch in range(100):
            ...
            for i, images in enumerate(self.train_loader):
                ...
                i += 1
                labels_real = Variable(torch.FloatTensor(np.ones(images[0].size(0)))).cuda(2)
                #labels_fake = Variable(torch.FloatTensor(np.zeros(self.batch_size))).cuda(2)

                image_class = Variable(images[1].cuda(2))
                np_image_class = image_class.cpu().data.numpy()
                images = images[0].cuda(2)


                labels_real = labels_real.cuda(2)
                #labels_fake = labels_fake.cuda(2)
                ...
                #clsn_mask = Variable(torch.zeros(self.batch_size)).cuda(2).long()
                #clsp_mask = Variable(torch.zeros(self.batch_size)).cuda(2).long()
                cls0_mask = Variable(torch.FloatTensor(np.zeros(images.size(0)))).cuda(2)
                cls1_mask = Variable(torch.FloatTensor(np.ones(images.size(0)))).cuda(2)
                cnt_pos = 0
                #cnt_neg = 0
                #nZeroIdx = []
                #nOneIdx = []
                ...
                #img_batch_var = Variable(torch.FloatTensor(p_img_batch.copy())).cuda(2)
                #mask_batch_var = Variable(torch.FloatTensor(p_mask_batch.copy())).cuda(2)
                ...
                loss_generator = loss_fake_real + loss_cls + 30* loss_l1 #+ 0.1*
                ...
                if (i % len(self.train_loader) == 0) and (i!=0):
                    ...
                    for im, la in self.test_loader:
                        ...
                        label_mask = Variable(torch.zeros(la.size(0), 50), volatile=True).cuda(2)
                        label_mask_i = Variable(torch.zeros(la.size(0), 50), volatile=True).cuda(2)
                        ...
                        if j % 100 == 0:
                            torchvision.utils.save_image(img_test_resized.data.cpu(),
                                                         './data/table1_img50/res50/epoch%dimages_%d.jpg' % (epoch + 1, j))
                            torchvision.utils.save_image(mask_test.data.cpu(),
                                                         './data/table1_img50/res50/epoch%dnoise_%d.jpg' % (epoch + 1, j))
                            torchvision.utils.save_image(reconst_images.data.cpu(),
                                                         './data/table1_img50/res50/epoch%dreconst_images_%d.jpg' % (epoch + 1, j))
                    correct_meanscore /= len(self.test_loader)
                    correct_meanscore_i /= len(self.test_loader)

                    print('Test Accuracy of the cls on the masked images for resnet50: %.4f %% - %.4f %% - %.4f' % (100 * correct / total, 100* correct_o / total, correct / correct_o))
                    print('Mean Accuracy: %.4f' % correct_meanscore.data[0])
                    ...
                    if correct / total > best_score:
                        ...
                        print("saving best model...")
                        torch.save(self.encoder.state_dict(), './data/table1_img50/res50/best-encoder.pth')
                        torch.save(self.decoder.state_dict(), './data/table1_img50/res50/best-decoder.pth')
                        torch.save(self.discriminator.state_dict(), './data/table1_img50/res50/best-discriminator.pth')
                        torch.save(self.optim_G_dis.state_dict(), './data/table1_img50/res50/best-optimizer.pth')
