...
class Trainer(object):
    def __init__(self, trainloader, testloader):
        ...
        self.pre_cnn_path = os.path.join('/home', 'yjyoo', 'Code', 'NoiseGAN-blackbox_enhance', 'data',
        ...
        #
        self.pre_cnn_2_path = os.path.join('/home', 'yjyoo', 'Code', 'NoiseGAN-blackbox_enhance', 'data',
    ...
    def train_adversarial(self):
        ...
        for epoch in range(100):
            ...
            for i, images in enumerate(self.train_loader):
                ...
                for index in range(self.batch_size):
                    ...
                    if cls_class.cpu().data.numpy()[index] == np_image_class[index]: # if real true
                        ...
                        if cls_class_o.cpu().data.numpy()[index] != np_image_class[index]:
                            cls0_mask[index] = 1
                            cnt_pos+=1
                            img_pos = images_resized.cpu().data.numpy()[index].copy()
                            mask_pos = mask.cpu().data.numpy()[index].copy()
                            if num_pbank<self.maxP:
                                self.p_bank.append(img_pos.copy())
                                self.pm_bank.append(mask_pos.copy())
                                num_pbank+=1
                            else:
                                pidx = np.random.permutation(self.maxP)[0]
                                num_pbank += 1
                                self.p_bank[pidx] = img_pos.copy()
                                self.pm_bank[pidx] = mask_pos.copy()
                        else:
                            nOneIdx.append(index)
                        ...
                        if num_nbank < self.maxN:
                            self.n_bank.append(img_pos.copy())
                            self.nm_bank.append(mask_pos.copy())
                            num_nbank += 1
                        else:
                            nidx = np.random.permutation(self.maxN)[0]
                            self.n_bank[nidx] = img_pos.copy()
                            self.nm_bank[nidx] = mask_pos.copy()
                            num_nbank += 1
                        nZeroIdx.append(index)


                p_img_batch = images_resized.cpu().data.numpy()
                p_mask_batch  = mask.cpu().data.numpy()

                n_img_batch = images_resized.cpu().data.numpy()
                n_mask_batch = mask.cpu().data.numpy()

                #renew the batch - negative
                if num_nbank > self.batch_size:
                    bankIdx = np.random.permutation(min(num_nbank, self.maxN))
                    for rev_id in range(self.batch_size):
                       n_img_batch[rev_id] = self.n_bank[bankIdx[rev_id]].copy()
                       n_mask_batch[rev_id] = self.nm_bank[bankIdx[rev_id]].copy()
                       clsn_mask[rev_id] = 0


                if num_pbank > self.batch_size:
                    bankIdx = np.random.permutation(min(num_pbank, self.maxP))

                    for rev_id in range(self.batch_size):
                        p_img_batch[rev_id] = self.p_bank[bankIdx[rev_id]].copy()
                        p_mask_batch[rev_id] = self.pm_bank[bankIdx[rev_id]].copy()
                        clsp_mask[rev_id] = 1

                pos_batch = p_img_batch + self.plambda*p_mask_batch
                neg_batch = n_img_batch + self.plambda*n_mask_batch

                #negative batch
                n_img_batch_var = Variable(torch.FloatTensor(neg_batch.copy())).cuda()
                nlogit_real = self.discriminator(n_img_batch_var.detach()) #image_result because the logit value is the result from..
                nloss_real_real = self.criterion_D(nlogit_real[0], clsn_mask)

                #positive batch
                p_img_batch_var = Variable(torch.FloatTensor(pos_batch.copy())).cuda()
                plogit_real = self.discriminator(p_img_batch_var.detach())  # image_result because the logit value is the result from..
                ploss_real_real = self.criterion_D(plogit_real[0], clsp_mask)
                ...
                zero_mask= Variable(torch.zeros(self.batch_size, 1)).cuda()
                for index in range(self.batch_size):
                    for cls in range(10):
                        if cls != np_image_class[index]:
                            one_mask[index, cls] = 1
                cnn_out_false = one_mask.detach() * func.softmax(cnn_out)
                cnn_out_false = torch.sum(cnn_out_false, dim=1)

                loss_cls = self.criterion_G_CNN(cnn_out, image_class.detach())
                loss_else= self.criterion_L1(cnn_out_false, zero_mask.detach()) #false data to be zero


                loss_generator = loss_fake_real  + loss_cls + loss_l1 + loss_else
                ...
                if i % 50 == 0:
                    print(
                    'Epoch [%d/%d], Step[%d/%d], loss_real_real: %.4f, loss_generator: %.4f, loss_cls: %.4f, loss_l1: %.4f, loss_else: %.4f, num cnt: %d'
                    % (epoch + 1, epoch, i, total_step, loss_discriminator.data[0], loss_fake_real.data[0],
                       loss_cls.data[0], loss_l1.data[0], loss_else.data[0], cnt_tot))
