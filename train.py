    ...
    def train_adversarial(self):
        ...
        for epoch in range(100):
            ...
            for i, images in enumerate(self.train_loader):
                ...
                cls1_mask = Variable(torch.zeros(self.batch_size)).cuda().long()
                ...
                for index in range(self.batch_size):
                    ...
                    if cls_class.cpu().data.numpy()[index] != cls_class_o.cpu().data.numpy()[index]: # if real true
                        cnt_all+=1
                        if cnt_all > 0: #always pass
                            cls0_mask[index] = 1
                            cnt_pos+=1
                ...
                if cnt_tot >= 0: #if discriminator is trained at least once
                    ...
                    if -1*num_cbank > self.maxC: # if discrminator 2 runs
                        #gan 2
                        #clogit_fake_n = self.discriminator_cls(image_result)
                        #closs_fake_real = self.criterion_D(clogit_fake_n, labels_real)
                        #loss_generator =  closs_fake_real  #+ loss_fake_real + loss_cls# + style_loss #initially we set weights as 1

                        if i % 50 == 0:
                            print(
                            'Epoch [%d/%d], Step[%d/%d], loss_real_real: %.4f, loss_fake_real: %.4f,  num cnt: %d'
                            % (epoch + 1, epoch, i, total_step, loss_discriminator.data[0], loss_fake_real.data[0], cnt_tot))
                            cnt_tot = 0
                    else:
                        loss_generator = loss_fake_real #+loss_l1

                        if i % 50 == 0:
                            print(
                                'Epoch [%d/%d], Step[%d/%d], loss_real_real: %.4f, loss_generator: %.4f,  num cnt: %d'
                                % (epoch + 1, epoch, i, total_step, loss_discriminator.data[0], loss_generator.data[0],
                                   cnt_tot))
                            cnt_tot = 0
